...
- `JSArrayBuffer`: the backing store is a mutable PHP binary string (`$bytes`), written a handful of bytes at a time via direct offset assignment rather than `substr_replace`. A string is the only heap-safe way to hold this data under §11's constraint — never a `SplFixedArray` or a resource. `JSTypedArray` (all nine kinds — one class, since they differ only in element size and a `match` over how a byte range decodes to a number) and `JSDataView` are views over one of these; neither stores its own bytes. `JSTypedArray` is Integer-Indexed exotic (10.4.5): every in-range index is present, writable, enumerable and non-configurable-to-change, indices never populate `$props`, and both a detached buffer and an out-of-range/non-integer canonical numeric index resolve to `undefined` *without* falling through to the prototype chain — getting that `$found` semantics right (own-but-undefined, not not-found) is what keeps a `%TypedArray%.prototype` accessor from being observably reachable through a bad index.
