...
- **Ahead-of-time PHP for the hot path** is planned but unbuilt: `docs/aot-php.md`. It fits without a new dispatch mechanism, because `BuiltinRegistry`'s string-ID indirection (forced by §11.3) is already the function table such a scheme needs. The open question is whether it earns its complexity once the library gaps above are closed.
