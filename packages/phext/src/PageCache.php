...
/**
 * Render once, on the first request, then serve a file — and re-render when
 * the entry gets too old.
 *
 * Incremental static regeneration, arranged for the hosting this is aimed at:
 * PHP only, no daemon, no background worker, no shared memory beyond opcache.
 * So there is nothing clever here — a hit is a file that exists and is young
 * enough, and a miss renders, writes, and serves.
 *
 * **No stale-while-revalidate**, deliberately. Serving a stale page while
 * something else refreshes it needs a background worker, and this runtime is
 * shared-nothing by construction (DESIGN.md §11) — there is nothing to be in
 * the background *of*. An expired entry is re-rendered by the request that
 * found it expired, and that request waits. What that costs is one slow
 * request per TTL per page; what it buys is a cache with no moving parts,
 * which is the only kind that works on hosting where you cannot run a daemon.
 *
...
final class PageCache
{
    ...
    {
        return <<<HTACCESS
        # Generated by phext. Serve a cached page without starting PHP.
